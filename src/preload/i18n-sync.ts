...
const i18nSync = async (scanPath = dfScanPath, options: Options = {}) => {
 ...
 console.log('appPath', appPath);
    ...
    const scanTranslateFile = async () => {
        ...
        const files = glob.sync(pattern).filter((file: string) => !file.endsWith('.d.ts'));
    ...
    const saveScanedFile = async (serverPhrashes: {}, scanedResult: { [x: string]: any; }) => {
        ...
        const filenameOutput = rootApp + `/${`${nowStr}-${timestampStr}-web-locale.json`}`;
    ...
    const log = (level: string | number, message: string, ...optionalParams: any[]) => {
        const levels = {
